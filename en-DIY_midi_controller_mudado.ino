...
#endif
// ---- //
// declarando as variáveis pra "mudar o canal"
...
const int BUTTON_ARDUINO_PIN[N_BUTTONS] = {5,4,3,2,9,8,7,6,13,12,11,10}; //* pins of each button connected straight to the Arduino

int buttonCState[N_BUTTONS] = {};        // stores the button current value
int buttonPState[N_BUTTONS] = {};        // stores the button previous value

#define pin13 1 //* uncomment if you are using pin 13 (pin with led), or comment the line if not using
byte pin13index = 8; //* put the index of the pin 13 of the buttonPin[] array if you are using, if not, comment
...
const int POT_ARDUINO_PIN[N_POTS] = {A5,A4 ,A3, A2, A1, A0}; //* pins of each pot connected straight to the Arduino
...
#ifdef ATMEGA328
// use if using with ATmega328 (uno, mega, nano...)
MIDI.sendNoteOn(note + i + 16*cannoli, 127, midiCh); // note, velocity, channel
...
#ifdef ATMEGA328
// use if using with ATmega328 (uno, mega, nano...)
MIDI.sendNoteOn(note + i + 16*cannoli, 0, midiCh); // note, velocity, channel
...
//Serial.print("  ");
      ...
      cannal = analogRead(POT_ARDUINO_PIN[0]);
      cannoli = map(cannal,0,1023,0,5);
